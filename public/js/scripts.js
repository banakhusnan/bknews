                ...
                if(index){
                    ...
                    if(role == 'author'){
                        ...
                        if(label.text() == 'Author'){
                            $("input[name='no_induk']").on("keyup", function(){
                                const noInduk = $(this).val();
                                if(noInduk.split(".")[0] != '06'){
                                    $('#labelNotif').html('<small class="text-danger" id="labelNotif">Kamu bukan author</small>');
                                    buttonReg.prop('disabled', true);
                                } else {
                                    $('#labelNotif').html('<small class="text-primary" id="labelNotif">Masukan no induk menggunakan (.)</small>')
                                }
                            });
                        }
                        ...
                        if(label.text() == 'Mahasiswa'){
                            $("input[name='no_induk']").on("keyup", function(){
                                ...
                                if(noInduk.split(".")[0] != '20'){
                                    $('#labelNotif').html('<small class="text-danger" id="labelNotif">Kamu bukan mahasiswa</small>');
                                    buttonReg.prop('disabled', true);
                                } else {
                                    $('#labelNotif').html('<small class="text-primary" id="labelNotif">Masukan no induk menggunakan (.)</small>')
                                }
